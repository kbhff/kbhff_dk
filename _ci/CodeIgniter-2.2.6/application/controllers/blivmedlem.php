		...
		else
		{
			...
			if ($viewdata['admin'])
			{
				...
				if ($this->input->post('divisionexists') > 0)
				{
					...
					$orderinfo = $this->_createorder($member_id, FF_MEMBERSHIP, $this->session->userdata('uid'), 1, 'now()', $method, $viewdata['division']);
				}
				$this->Personsmodel->send_new_membermail($member_id);
				$viewdata['title'] = 'Nyt medlem registreret, kontant/mobilepay betaling';
				$viewdata['heading'] = 'Nyt medlem registreret, kontant/mobilepay betaling';
