...
if ($status <> 'paid')
{
	echo "<strong>Jeg er medlem, men har ikke betalt kontingent endnu.</strong><br>";
}
?>
<br>
<strong>Bestilling af poser:</strong><br>
<img src="/img/posebillede.jpg" alt="KBHFF har gode poser!" width="100" height="120" border="0" align="right"><table class="picuplist">
...
<? if ($status <> 'paid') { ?>
<tr class="picuplist"><td>&nbsp;</td><td>2017</td><td><strong>KBHFF Kontingent</strong></td>
<td class="rjustify"><input type="hidden" name="item[]" value="<?= $annualfee[0]['id'] ?>"><input type="hidden" name="pickupdate[]" value=""><input type="hidden" class="itamount" name="itemamount[]" value="<?= $annualfee[0]['amount'];?>"><?= $annualfee[0]['amount'];?></td>
<td class="rjustify inorder"></td><td class="rjustify"><input type="text" name="quant[]" value="1" size="1" maxlength="1" readonly></td><td>&nbsp;</td></tr>
<?php }?>

<tr class="picuplist"><td>&nbsp;</td><td>&nbsp;</td><td><strong>Stofpose-andel</strong></td>
<td class="rjustify"><input type="hidden" name="item[]" value="<?= $bagfee[0]['id'] ?>"><input type="hidden" name="pickupdate[]" value=""><input type="hidden" class="itamount" name="itemamount[]" value="<?= $bagfee[0]['amount'];?>"><?= $bagfee[0]['amount'];?></td>
<td class="rjustify inorder"></td><td class="rjustify"><select onChange="calcsum()" class="quantity" name="quant[]"><?=$bag_quantity ?></select></td><td>&nbsp;</td></tr>
...
if ($item['cancel'] <0 ) { 
	echo '&nbsp;</td><td><input type="hidden" name="quant[]" value="0"><span class="disabled">' . $item['quant'] . '</span></td><td><img src="/img/exclamation.png" alt="For sent at &aelig;ndre" width="15" height="15" border="0" align="right"></td>';
	} else {
...
<img src="/img/exclamation.png" alt="For sent at &aelig;ndre" width="15" height="15" vspace="2" border="0" align="left">: For sent at &aelig;ndre denne ordre.<br clear="left"><br>
