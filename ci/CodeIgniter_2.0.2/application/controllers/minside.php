...
//		$this->load->model('Memberinfo');
		...
		{
			$status = 'Jeg er medlem, og har betalt medlemskontingent.<br>'; 
		} else {
			$status = '<strong>Jeg er medlem, men har <em>ikke</em> betalt medlemskontingent endnu.</strong><br>'; 
		}
		$data = array(
...
//		print "USER:" . $user . "<br>\n";
		...
		if (($return_code === 'yes')||($return_code === 'paid'))
		{
				...
				$this->session->set_userdata(('active'), $return_code);
