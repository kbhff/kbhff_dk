...
$config['smtp_pass'] = 'robertKlump4';
$config['smtp_port'] = '465';
$config['crlf'] = '\r\n';
$config['newline'] = '\r\n';
$config['mailtype'] = 'html';
$config['smtp_host'] = 'ssl://mail.skrig.dk';
$config['bcc_batch_mode'] = TRUE;
$config['bcc_batch_size'] = 10;
...
if ($subsel == 'a')
{
	...
	$select = ' where ff_persons.email > "" ' . $privsel . ' AND ((ff_persons.active = "paid")or(ff_persons.active = "paid")) AND ff_division_members.member = ff_persons.uid AND ff_division_members.division = ' . (int)$division;
}
...
if (substr($subsel,0,1) == 'g')
{
	...
	$q = 'SELECT email FROM ff_persons, ff_division_members' . $select . ' AND ((ff_persons.active = "paid")or(ff_persons.active = "paid")) AND ff_division_members.member = ff_persons.uid AND ff_division_members.division = ' . (int)$division;
	$q .= ' union ';
	$q .= 'SELECT alias FROM ff_persons, ff_division_members, ff_mail_aliases' . $select . ' AND ff_mail_aliases.puid = ff_persons.uid AND ff_persons.active = "paid" AND ff_division_members.member = ff_persons.uid AND ff_division_members.division = ' . (int)$division;
...
if ((intval($this->session->userdata('uid'))<10)&&($division == 999))	// member is superadmin & all depts
{
	...
	$q = 'SELECT email FROM ff_persons' . $select . ' AND ((ff_persons.active = "paid")or(ff_persons.active = "paid"))';
	$q .= ' union ';
	$q .= 'SELECT alias FROM ff_persons, ff_mail_aliases' . $select . ' AND ff_mail_aliases.puid = ff_persons.uid AND ((ff_persons.active = "paid")or(ff_persons.active = "paid"))';
}
