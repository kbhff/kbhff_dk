...
/*i-glemt.js*/
Util.Objects["glemt"] = new function() {
	this.init = function(scene) {
		scene.resized = function() {
		}
		scene.scrolled = function() {
		...
		scene.ready = function() {
			...
			form.submitted = function() {
				var data = u.f.getParams(this, {"send_as":"json"}); 
				var validData = ["12345", "11223344", "email@email.dk"]; 
				console.log(data);
				console.log("Returned index: " + validData.indexOf(data.login));
				if (!u.qs("form.verify", this) && validData.indexOf(data.login) !== -1) {
					var form_verify = u.f.addForm(this.parentNode, {"class":"form verify", "action":"#", "method":"POST"}); 
					...
					var button_verify = u.f.addAction(fieldset_verify, {"name":"save", "type":"submit", "value":"lav nyt kodeord", "class":"button primary"});
					...
					form = u.qs("form.verify");
					u.f.init(form);
					data = u.f.getParams(form, {"send_as":"json"});
				}
				else { 
					if (!u.qs("div.notfound", this)) {
						var div_notfound = u.ie(u.qs("fieldset.login"), "div", {"class":"notfound"});
