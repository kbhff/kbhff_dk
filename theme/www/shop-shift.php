...
// page info
$page->bodyClass("shop-shift");
$page->pageTitle("FRIVILLIG");

if($action) {
	
	 if($action[0] == "soeg") {
	 	$orders = $model->searchOrders(array("status" => getPost("status"), "search_pattern" => getPost("search")));
	 	$search_pattern = getPost("search");
	 
	 	if($orders) {
	 		message()->resetMessages();
	 		message()->addMessage("søgeresultat!! ".count($orders), array("type" => "error"));
	...
	// /medlemshjaelp/paymentError
		...
		$page->page(array(
			"templates" => "shop-shift/payment.php"
		));
		exit();
	}
