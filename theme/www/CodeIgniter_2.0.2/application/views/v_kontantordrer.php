...
foreach ($pickups as $item)
{
	...
	echo ("\n\n" . '<br><input type="hidden" name="item[]" value="' . $bagfee[0]['itemid']. '">' . "\n");
	echo ('<input type="hidden" name="pickupday[]" value="0">' . "\n");
	echo ('<select name="quant[]">' . $bag_quantity . '</select> ' . "\n");
	echo ($bagfee[0]['explained'] . ', ' . $bagfee[0]['amount'] . ' kr.<br>');

	$count++;

	echo ("\n\n" .'<br><input type="hidden" name="item[]" value="' . $annualfee[0]['itemid']. '">' . "\n");
	echo ('<input type="hidden" name="pickupday[]" value="0">' . "\n");
	echo ('<select name="quant[]"><option value="0">0</option><option value="1">1 stk</option></select> ');
	echo ($annualfee[0]['explained'] . ', ' . $annualfee[0]['amount'] . ' kr.<br>');
		...
		foreach ($members as $member)
		{
			...
			switch ($activestatus) {
				...
				case '2' :
					...
					echo '			<td>'.$member['uid'].'</td>'."\n";
					echo '			<td><strong>Opkr&aelig;v kontingent!</strong> '.$member['name'].'</td>'."\n";
					break;
				case '3' :
					echo '			<td><input type="radio" name="puid" value="'.$member['uid'] .'" /></td>'."\n";
					echo '			<td>'.$member['uid'].'</td>'."\n";
				...
				case '4' :
					echo '			<td>&nbsp;</td>'."\n";
					echo '			<td>'.$member['uid'].'</td>'."\n";
					echo '			<td>(udmeldt: '.$member['name'].')</td>'."\n";
					break;
			}
...
?>
	...
	<br>
	<input type="submit" value="Registrer kontant" name="kontantordre"  class="form_button"/>
	<input type="submit" value="Registrer mobilepay" name="mobilepay"  class="form_button"/><br>
	</form>
