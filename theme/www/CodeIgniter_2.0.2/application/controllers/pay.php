...
//	$this->javascript->compile();
	...
	if (is_array($item)) // meaning, more than table-line
	{
		...
		$bagorders =  $this->_getbagorders($orderno) ;
		$otherorders =  $this->_getotherorders($orderno) ;
...
//		$js =$this->jquery->corner('#tt_del');
		...
		if ($this->_getannualfeeorders($OrderID)>0)
		{
			$annualfeepaid = TRUE;
			$this->session->set_userdata(('active'), 'paid');
			$data = array(
               'active' => 'paid'
            );
    ...
    function _getbagorders($orderno) 
	{
    ...
    function _getotherorders($orderno) 
	{
		$this->db->select('ff_orderlines.item, ff_orderlines.quant, ff_orderlines.amount, ff_producttypes.explained, ff_items.measure')
		->from('ff_orderlines')
		->from('ff_producttypes')
		->from('ff_items')
		;
