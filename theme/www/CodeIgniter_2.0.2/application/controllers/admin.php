...
class Admin extends CI_Controller {
	...
	function dagens_salg($division=5,$dag = 0)
	{
		...
		if ($dag == 0)
		{
			...
			$modtagnekontanter = $this->_dagens_modtagne_kontanter($division,$dagsdato, 'kontant');
			$modtagnemobilepay = $this->_dagens_modtagne_kontanter($division,$dagsdato, 'mobilepay');
			$ikkeafhentet = $this->_ikkeafhentet($division,$dagsdato);

			$kontant = $this->_dagens_kontantsalg($dag, 'kontant');
			$mobilepay = $this->_dagens_kontantsalg($dag, 'mobilepay');
			...
			$data = array(
				   ...
				   'modtagnemobilepay' => $modtagnemobilepay,
				   'ikkeafhentet' => $ikkeafhentet,
				   'nets' => $nets,

				   'kontant_groent' => $this->_modtagne_kontanter('kontant',$division,$dagsdato, FF_GROCERYBAG),
				   'kontant_frugt' => $this->_modtagne_kontanter('kontant',$division,$dagsdato, FF_FRUITBAG),
				   'kontant_asparges' => $this->_modtagne_kontanter('kontant',$division,$dagsdato, FF_ASPARAGUS),
				   'kontant_stofpose' => $this->_modtagne_kontanter('kontant',$division,$dagsdato, FF_BAGFEE),
				   'kontant_indmeldelse' => $this->_modtagne_kontanter('kontant',$division,$dagsdato, FF_MEMBERSHIP),
				   'kontant_kontingent' => $this->_modtagne_kontanter('kontant',$division,$dagsdato, FF_ANNUALFEE),

				   'mobilepay_groent' => $this->_modtagne_kontanter('mobilepay',$division,$dagsdato, FF_GROCERYBAG),
				   'mobilepay_frugt' => $this->_modtagne_kontanter('mobilepay',$division,$dagsdato, FF_FRUITBAG),
				   'mobilepay_asparges' => $this->_modtagne_kontanter('mobilepay',$division,$dagsdato, FF_ASPARAGUS),
				   'mobilepay_stofpose' => $this->_modtagne_kontanter('mobilepay',$division,$dagsdato, FF_BAGFEE),
				   'mobilepay_indmeldelse' => $this->_modtagne_kontanter('mobilepay',$division,$dagsdato, FF_MEMBERSHIP),
				   'mobilepay_kontingent' => $this->_modtagne_kontanter('mobilepay',$division,$dagsdato, FF_ANNUALFEE),
	          );
			$this->load->view('v_dagens_salg', $data);
		}
	...
	function _modtagne_kontanter($method, $division,$dagsdato, $id)
	{
		$kontant = '';
		$sql = 'SELECT sum( ff_orderlines.amount ) AS amountsum 
				FROM ff_orderlines, ff_orderhead, ff_items, ff_producttypes,  ff_divisions
				WHERE ff_orderlines.orderno = ff_orderhead.orderno
				AND ff_orderhead.status1 = "' . $method . '" 
				AND ff_orderlines.item = ff_items.id
				AND ff_items.producttype_id = ff_producttypes.id
				AND ff_items.producttype_id = ' . $id . '
				AND ff_items.division = ' . $division . '
				AND ff_items.division = ff_divisions.uid
				AND DATE_FORMAT(ff_orderhead.created,"%Y-%m-%d") = "' . addslashes($dagsdato) . '" ';
...
//		$cellval = trim(iconv("UTF-8","ISO-8859-1",$cell->getValue())," \t\n\r\0\x0B\xA0");
	...
	function _dagens_kontantsalg($divisionday, $method = 'ukendt')
	{
		$kontant = '';
...
ORDER BY ff_orderlines.created,ff_producttypes.explained');
			...
			if ($query->num_rows() > 0)
			{
				...
				$kontant = 'Intet ' . $method. ' salg.<br>';
			}
		return $kontant;
	}

	function _dagens_modtagne_kontanter($division,$day, $method = 'ukendt')
	{
		$kontantermodtaget  = '';
			$query = $this->db->query('SELECT sum( ff_orderlines.quant ) AS quantsum , sum( ff_orderlines.amount ) AS amountsum , ff_orderlines.item, ff_items.units, ff_items.measure, ff_producttypes.explained, DATE_FORMAT(ff_orderlines.created,"%d-%m-%Y") as created
...
ORDER BY ff_orderlines.created, ff_orderlines.item');
			...
			if ($query->num_rows() > 0)
			{
				...
				$kontantermodtaget = 'Intet ' . $method. ' salg.<br>';
			}
...
ORDER BY ff_producttypes.explained');
	...
	function _dagens_mobilepaysalg($divisionday)
	{
		$nets = '';
			$query = $this->db->query('SELECT sum( ff_orderlines.quant ) AS quantsum , sum( ff_orderlines.amount ) AS amountsum , ff_orderlines.item, ff_items.units, ff_items.measure, ff_producttypes.explained, ff_orderlines.created
FROM ff_orderlines, ff_orderhead, ff_items, ff_producttypes, ff_pickupdates, ff_divisions
WHERE ff_orderlines.orderno = ff_orderhead.orderno
AND ff_orderhead.status1 = "mobilepay"
AND ff_orderlines.item = ff_items.id
AND ff_items.producttype_id = ff_producttypes.id
AND ff_orderlines.iteminfo = ff_pickupdates.uid
AND ff_divisions.uid = ff_pickupdates.division
