...
class Memberinfo extends CI_Model {
	...
	function annualfee_uid_by_division($division)
	{

		$this->db->select('items.id id');
		$this->db->select('producttypes.id as itemid');
		$this->db->select('items.amount as amount');
		$this->db->select('producttypes.explained');
		$this->db->from('items');
		$this->db->from('producttypes');
		$this->db->where('producttypes.id', FF_ANNUALFEE );
		$this->db->where('ff_items.producttype_id', "ff_producttypes.id", FALSE);
		$this->db->where('items.division', (int)$division); 
		$query = $this->db->get();
	...
	function bagfee_uid_by_division($division)
	{
		...
		$this->db->from('items');
		$this->db->from('producttypes');
		$this->db->where('producttypes.id', FF_BAGFEE );
		$this->db->where('ff_items.producttype_id', "ff_producttypes.id", FALSE);
		$this->db->where('items.division', (int)$division); 
		$query = $this->db->get();
...
/*
		...
		$this->db->join('orderhead', 'orderhead.orderno = orderlines.orderno 
		AND ((' . $this->db->protect_identifiers('orderhead', TRUE) . '.status1 = "nets") 
		or (' . $this->db->protect_identifiers('orderhead', TRUE) . '.status1 = "kontant") 
		or (' . $this->db->protect_identifiers('orderhead', TRUE) . '.status1 = "mobilepay")) 

		AND ' . $this->db->protect_identifiers('orderhead', TRUE) . '.puid = ' . $this->db->protect_identifiers('division_members', TRUE) . '.member', 'left');		
...
ORDER BY `ff_pickupdates`.`pickupdate`,ff_producttypes.explained ');
    ...
    if($query->num_rows() === 1 && $this->phpass->check($pw, $row->password) && $timestamp > mysql_to_unix($row->last_login))
		{
			if (($row->active == 'yes')||($row->active == 'paid'))
			{
