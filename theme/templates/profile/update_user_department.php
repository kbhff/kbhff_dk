...
$unshipped_order_items = $SC->getOrderItems(["user_id" => session()->value("user_id"), "where" => "shipped_by IS NULL AND items.itemtype REGEXP '^(legacy)?product'"]);
...
<div class="scene update_userinfo_form i:update_userinfo_form">
	...
	<?= $UC->formStart("updateUserDepartment", ["class" => "form_department"]) ?> 
		...
		<ul class="actions">
			...
			<?= $UC->submit("Opdater", array("class" => "primary disabled", "wrapper" => "li.save")) ?>
		</ul>
